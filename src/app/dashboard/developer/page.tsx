        ...
        <Section icon={Beaker} title="Generate Sample Data" iconColor="text-rose-500">
          ...
          <Button variant="outline" onClick={async () => {
            ...
            for (let i = 0; i < 50; i++) {
              ...
              samples.push(mk(i % 2 === 0 ? 'expense' : 'income', i % 2 === 0 ? 'Food' : 'Freelance', `Sample #${i + 1}`, d.toISOString().split('T')[0], Math.floor(Math.random() * 5000) + 100));
            }
            await db.transactions.bulkAdd(samples as any);
            try { const c = JSON.parse(localStorage.getItem('mm_transactions') || '[]'); localStorage.setItem('mm_transactions', JSON.stringify([...c, ...samples])); } catch {}
